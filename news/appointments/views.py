...
class AppointmentView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        send_mail(
            ...
            recipient_list=['piter_pen1999@mail.ru',]  # здесь список получателей. Например, секретарь, сам врач и т. д.
        )
    ...
    def post(self, request):
        ...
        for category in Category.objects.all():
            ...
            for news in Post.objects.filter(category_id=category.id,
                                            createpost_datetime__week=datetime.now().isocalendar()[1] - 1).values('pk',
                                                                                                           'head_text',
                                                                                                           'createpost_datetime'):
                new = (f'{news.get("head_text")}, {news.get("createpost_datetime")}, http://127.0.0.1:8000/news/{news.get("pk")}')
                news_from_each_category.append(new)
            print("Письма отправлены подписчикам категории:", category.id, category.name_category)
